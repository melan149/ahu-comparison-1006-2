...
# Specific column to trigger chart display below it in the table
# Renamed from internal_width_supply_fan_col to reflect new chart placement logic
...
for i in range(1, 6): # For X1, Y1 to X5, Y5
    ...
    # else:
        # st.sidebar.warning(f"Warning: Coordinate column 'X{i}' or 'Y{i}' not found in data. Chart may be incomplete.")
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    for col in df.columns:
        ...
        if col not in excluded_cols:
            ...
            # Insert chart after "Unit cross section area (Supply Filter)" row
            if col == unit_cross_section_area_supply_filter_col and not chart_inserted:
                ...
                if can_plot_brand1:
                    for i, (x_name, y_name) in enumerate(coord_col_pairs):
                        chart_data.append({
                            'X_coord': filtered_df1[x_name].values[0] / 20.0, # Scaled value for plotting
                            'Y_coord': filtered_df1[y_name].values[0] / 20.0, # Scaled value for plotting
                            'Display_Label': f"{selected_brand1} - {selected_unit1}", # Combined label for legend
                ...
                if can_plot_brand2:
                    for i, (x_name, y_name) in enumerate(coord_col_pairs):
                        chart_data.append({
                            'X_coord': filtered_df2[x_name].values[0] / 20.0, # Scaled value for plotting
                            'Y_coord': filtered_df2[y_name].values[0] / 20.0, # Scaled value for plotting
                            'Display_Label': f"{selected_brand2} - {selected_unit2}", # Combined label for legend
                            'Point_Order': i + 1 # Point order 1 to 5
                        })
                ...
                if chart_data: # Only attempt to plot if some data is gathered
                    ...
                    chart_df = chart_df.sort_values(by=['Display_Label', 'Point_Order'])
                    
                    # Create the Plotly chart
                    fig = px.line(chart_df,
                                  x="X_coord",
                                  y="Y_coord",
                                  color="Display_Label", # Use combined label for color
                                  line_group="Display_Label", # Group lines by combined label
                                  markers=True, # Show markers at data points
                                  title=None) # Hide chart title as requested
                    
                    # Update layout for better visualization and axis titles
                    fig.update_layout(
                        xaxis_title="Unit internal width_Supply Filter (mm)", # Renamed X-axis title
                        yaxis_title="Unit internal height_Supply Filter (mm)", # Renamed Y-axis title
                        hovermode="x unified",
                        legend_title_text="Brand - Unit Name", # Renamed legend title
