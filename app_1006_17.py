...
# Specific column to trigger chart 1 display below it in the table
...
# Specific column to trigger chart 2 display below it in the table
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    excluded_cols = [
        ...
        unit_cross_section_area_supply_filter_col, # Exclude this as it's the trigger for Chart 1
        internal_height_supply_fan_col # Exclude this as it's the trigger for Chart 2
    ]
    ...
    chart1_inserted = False
    chart2_inserted = False
    ...
    for col in df.columns:
        if col not in excluded_cols:
            ...
            # Insert Chart 1 after "Unit cross section area (Supply Filter)" row
            if col == unit_cross_section_area_supply_filter_col and not chart1_inserted:
                st.markdown("---")
                chart_data_1 = []
                ...
                if can_plot_brand1_chart1:
                    for i, (x_name, y_name) in enumerate(coord_col_pairs_1_5):
                        chart_data_1.append({
                            'X_coord': filtered_df1[x_name].values[0], # Keep original values for axis labels
                            'Y_coord': filtered_df1[y_name].values[0], # Keep original values for axis labels
                ...
                if can_plot_brand2_chart1:
                    for i, (x_name, y_name) in enumerate(coord_col_pairs_1_5):
                        chart_data_1.append({
                            'X_coord': filtered_df2[x_name].values[0], # Keep original values for axis labels
                            'Y_coord': filtered_df2[y_name].values[0], # Keep original values for axis labels
                ...
                if chart_data_1:
                    ...
                    fig1 = px.line(chart_df_1,
                                   x="Scaled_X_coord", # Use scaled coordinates for plotting
                                   y="Scaled_Y_coord", # Use scaled coordinates for plotting
                                   color="Display_Label",
                ...
                st.markdown("---")
                chart1_inserted = True

            # Insert Chart 2 after "Internal Height (Supply Fan)" row
            if col == internal_height_supply_fan_col and not chart2_inserted:
                st.markdown("---")
                chart_data_2 = []
                ...
                if can_plot_brand1_chart2:
                    for i, (x_name, y_name) in enumerate(coord_col_pairs_6_10):
                        chart_data_2.append({
                            'X_coord': filtered_df1[x_name].values[0],
                            'Y_coord': filtered_df1[y_name].values[0],
                ...
                if can_plot_brand2_chart2:
                    for i, (x_name, y_name) in enumerate(coord_col_pairs_6_10):
                        chart_data_2.append({
                            'X_coord': filtered_df2[x_name].values[0],
                            'Y_coord': filtered_df2[y_name].values[0],
                ...
                if chart_data_2:
                    ...
                    st.warning("No complete coordinate data (X6-X10, Y6-Y10) found for selected units to generate this geometry chart. Please ensure data is present and valid for both selections.")
                st.markdown("---")
                chart2_inserted = True
