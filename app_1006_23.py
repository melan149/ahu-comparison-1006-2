...
for potential_col_name, header_title in header_triggers_raw.items():
    actual_col_name = get_column_safe(df, [
        ...
        potential_col_name.replace("(","").replace(")","") # Added for specific column names
    ])
    if actual_col_name:
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    with col2:
        st.markdown(f"**{selected_brand1} - {selected_unit1} - {selected_size1}**") # Updated header format
    with col3:
        st.markdown(f"**{selected_brand2} - {selected_unit2} - {selected_size2}**") # Updated header format
    ...
    for col in df.columns:
        ...
        if col not in excluded_cols_from_table:
            ...
            with row_col2:
                st.write(f'<span style="font-family: sans-serif; font-size: 16px;">{val1}</span>', unsafe_allow_html=True) # Ensure consistent font for values
            with row_col3:
                st.write(f'<span style="font-family: sans-serif; font-size: 16px;">{val2}</span>', unsafe_allow_html=True) # Ensure consistent font for values


        # --- Insert Section Headers (AFTER the trigger row) ---
        if col in resolved_header_triggers and resolved_header_triggers[col] not in displayed_headers:
            st.markdown("---")
            # Use columns to center the header and apply styling
            header_col1, header_col2, header_col3 = st.columns([1, 4, 1])
            with header_col2:
                # Custom styling for header: reduced font size, centered
                st.markdown(f'<h4 style="text-align: center; font-size: 1.2em; margin-bottom: 0.5em; margin-top: 0.5em;">{resolved_header_triggers[col]}</h4>', unsafe_allow_html=True)
            
            # Re-add table headers for the new section below the centered header
            col1, col2, col3 = st.columns([2, 3, 3])
            with col1: st.markdown("**Parameter**")
            with col2: st.markdown(f"**{selected_brand1} - {selected_unit1} - {selected_size1}**") # Updated header format
            with col3: st.markdown(f"**{selected_brand2} - {selected_unit2} - {selected_size2}**") # Updated header format
            displayed_headers.add(resolved_header_triggers[col]) # Mark as displayed
        ...
        if col == duct_connection_height_col and not chart3_displayed:
            ...
            if not filtered_df1.empty:
                for coord_col in required_coord_cols_chart3:
                    if coord_col not in filtered_df1.columns or pd.isna(filtered_df1[coord_col].values[0]):
            ...
            if chart_data_3:
                ...
                st.warning("No complete coordinate data (X11-X15, Y11-Y15) found for selected units to generate Chart 3. Please ensure data is present and valid for both selections.")
            st.markdown("---")
