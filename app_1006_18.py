...
# Specific column to trigger chart 1 display below it in the table (not used for placement anymore)
unit_cross_section_area_supply_filter_col = get_column_safe(df, ["Unit cross section area (Supply Filter)", "Unit cross section area Supply Filter", "Unit cross section area (SupplyFilter)"])
# Specific column to trigger chart 2 display below it in the table (not used for placement anymore)
internal_height_supply_fan_col = get_column_safe(df, ["Internal Height (Supply Fan)", "Internal Height Supply Fan"])
...
for i in range(1, 6):
    ...
    # else:
    #     st.sidebar.warning(f"Warning: Coordinate column 'X{i}' or 'Y{i}' for chart 1 not found in data. Chart may be incomplete.")
...
for i in range(6, 11): # For X6, Y6 to X10, Y10
    ...
    # else:
    #     st.sidebar.warning(f"Warning: Coordinate column 'X{i}' or 'Y{i}' for chart 2 not found in data. Chart may be incomplete.")
...
# --- Chart 1 Generation (X1-X5, Y1-Y5) ---
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    if len(coord_col_pairs_1_5) != 5:
        st.warning("Cannot generate Chart 1: Not all 5 X,Y coordinate pairs (X1-X5, Y1-Y5) were found in the Excel data or mapped correctly.")
    else:
            ...
            for x_name, y_name in coord_col_pairs_1_5:
                if x_name not in filtered_df1.columns or pd.isna(filtered_df1[x_name].values[0]) or \
                   y_name not in filtered_df1.columns or pd.isna(filtered_df1[y_name].values[0]):
                    ...
                    st.info(f"Incomplete coordinate data (X1-X5, Y1-Y5) for 'Left: {selected_brand1} - {selected_unit1}'. Chart 1 may not include this selection.")
                    break
        else:
            can_plot_brand1_chart1 = False
            ...
            for x_name, y_name in coord_col_pairs_1_5:
                if x_name not in filtered_df2.columns or pd.isna(filtered_df2[x_name].values[0]) or \
                   y_name not in filtered_df2.columns or pd.isna(filtered_df2[y_name].values[0]):
                    ...
                    st.info(f"Incomplete coordinate data (X1-X5, Y1-Y5) for 'Right: {selected_brand2} - {selected_unit2}'. Chart 1 may not include this selection.")
                    break
        else:
            can_plot_brand2_chart1 = False
        ...
        if can_plot_brand1_chart1:
            for i, (x_name, y_name) in enumerate(coord_col_pairs_1_5):
                chart_data_1.append({
                    'X_coord_actual': filtered_df1[x_name].values[0], # Actual values for axis labels/hover
                    'Y_coord_actual': filtered_df1[y_name].values[0], # Actual values for axis labels/hover
                    'Scaled_X_coord': filtered_df1[x_name].values[0] / 20.0, # Scaled for plotting
                    'Scaled_Y_coord': filtered_df1[y_name].values[0] / 20.0, # Scaled for plotting
        ...
        if can_plot_brand2_chart1:
            for i, (x_name, y_name) in enumerate(coord_col_pairs_1_5):
                chart_data_1.append({
                    'X_coord_actual': filtered_df2[x_name].values[0], # Actual values for axis labels/hover
                    'Y_coord_actual': filtered_df2[y_name].values[0], # Actual values for axis labels/hover
                    'Scaled_X_coord': filtered_df2[x_name].values[0] / 20.0, # Scaled for plotting
                    'Scaled_Y_coord': filtered_df2[y_name].values[0] / 20.0, # Scaled for plotting
                    'Display_Label': f"Right: {selected_year2}-{selected_quarter2}-{selected_brand2}-{selected_unit2}-{selected_size2}",
                    'Point_Order': i + 1
                })
        
        if chart_data_1:
            st.subheader("Unit Geometry Comparison (X1-X5, Y1-Y5)")
            ...
            fig1 = px.line(chart_df_1,
                           ...
                           markers=True,
                           title=None,
                           hover_data={'X_coord_actual': True, 'Y_coord_actual': True, 'Scaled_X_coord': False, 'Scaled_Y_coord': False})
...
# --- Chart 2 Generation (X6-X10, Y6-Y10) ---
    chart_data_2 = []
    
    # Check if all 5 coordinate pairs were found during initial column resolution
    if len(coord_col_pairs_6_10) != 5:
        st.warning("Cannot generate Chart 2: Not all 5 X,Y coordinate pairs (X6-X10, Y6-Y10) were found in the Excel data or mapped correctly.")
    else:
            ...
            for x_name, y_name in coord_col_pairs_6_10:
                if x_name not in filtered_df1.columns or pd.isna(filtered_df1[x_name].values[0]) or \
                   y_name not in filtered_df1.columns or pd.isna(filtered_df1[y_name].values[0]):
                    ...
                    st.info(f"Incomplete coordinate data (X6-X10, Y6-Y10) for 'Left: {selected_brand1} - {selected_unit1}'. Chart 2 may not include this selection.")
                    break
        else:
            can_plot_brand1_chart2 = False
            ...
            for x_name, y_name in coord_col_pairs_6_10:
                if x_name not in filtered_df2.columns or pd.isna(filtered_df2[x_name].values[0]) or \
                   y_name not in filtered_df2.columns or pd.isna(filtered_df2[y_name].values[0]):
                    ...
                    st.info(f"Incomplete coordinate data (X6-X10, Y6-Y10) for 'Right: {selected_brand2} - {selected_unit2}'. Chart 2 may not include this selection.")
                    break
        else:
            can_plot_brand2_chart2 = False
        ...
        if can_plot_brand1_chart2:
            for i, (x_name, y_name) in enumerate(coord_col_pairs_6_10):
                chart_data_2.append({
                    ...
                    'Scaled_X_coord': filtered_df1[x_name].values[0] / 20.0,
                    'Scaled_Y_coord': filtered_df1[y_name].values[0] / 20.0,
                    'Display_Label': f"Left: {selected_year1}-{selected_quarter1}-{selected_brand1}-{selected_unit1}-{selected_size1}",
                    'Point_Order': i + 1
                })
        ...
        if can_plot_brand2_chart2:
            for i, (x_name, y_name) in enumerate(coord_col_pairs_6_10):
                chart_data_2.append({
                    ...
                    'Scaled_X_coord': filtered_df2[x_name].values[0] / 20.0,
                    'Scaled_Y_coord': filtered_df2[y_name].values[0] / 20.0,
                    'Display_Label': f"Right: {selected_year2}-{selected_quarter2}-{selected_brand2}-{selected_unit2}-{selected_size2}",
                    'Point_Order': i + 1
                })
        
        if chart_data_2:
            st.subheader("Unit Geometry Comparison (X6-X10, Y6-Y10)")
    ...
    excluded_cols = [
        ...
        unit_cross_section_area_supply_filter_col, # Exclude this from table display
        internal_height_supply_fan_col # Exclude this from table display
    ]
    ...
    # The charts are now generated before this loop, so chart_inserted flags are no longer needed here for insertion
    # They are still used within the chart generation blocks above.
