...
# Header trigger columns (using the exact string to match later)
header_triggers = {
    ...
    "Duct Connection Diameter": "Rotary wheel", # This is now for a header, not a chart trigger directly
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    for col in df.columns:
        ...
        # The exact string from header_triggers dictionary is used for direct comparison
        if col in header_triggers:
            st.markdown("---")
            st.subheader(header_triggers[col])
        ...
        # Display the row if it's not one of the internal coordinate columns
        # or a column whose content is handled by a header/chart placement logic
