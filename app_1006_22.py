...
header_triggers_raw = {
    ...
    "Duct connection Diameter": "Rotary wheel", # Added 'Duct connection Diameter' for Rotary wheel header
...
for potential_col_name, header_title in header_triggers_raw.items():
    # Use get_column_safe to find the actual column name in df.columns
    # Add common variations for column names if needed, similar to other safe getters
    actual_col_name = get_column_safe(df, [
        potential_col_name,
        potential_col_name.lower(), #lowercase
        potential_col_name.title(), #Title Case
        potential_col_name.replace(" ", "_"), # underscores
        potential_col_name.replace(" ", "").lower() # no spaces, lowercase
    ])
    if actual_col_name:
...
if not filtered_df1.empty and not filtered_df2.empty:
    ...
    for col in df.columns:
        ...
        if col in resolved_header_triggers and resolved_header_triggers[col] not in displayed_headers:
            st.markdown("---")
            st.subheader(resolved_header_triggers[col])
            # Re-add table headers for the new section
            col1, col2, col3 = st.columns([2, 3, 3])
            with col1: st.markdown("**Parameter**")
            with col2: st.markdown(f"**{selected_brand1}**")
            with col3: st.markdown(f"**{selected_brand2}**")
        ...
        if col == internal_height_supply_filter_col and not chart1_displayed:
            ...
            if chart_data_1:
                st.subheader("Internal Cross Section area (Supply Filter)") # Chart 1 Title
        ...
        if col == unit_cross_section_area_supply_fan_col and not chart2_displayed:
            ...
            if chart_data_2:
                st.subheader("Internal Cross Section area (Supply Fan)") # Chart 2 Title
        ...
        if col == duct_connection_height_col and not chart3_displayed:
            ...
            # Ensure the specific coordinate columns for chart 3 are present in the filtered dataframes
            ...
            if chart_data_3:
                st.subheader("Supply Duct connection, mm") # Chart 3 Title
